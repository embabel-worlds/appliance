...
/** @param {Settings} s */
const auth = (s: Settings) => 'Basic ' + Buffer.from(`${s.username}:${s.password}`).toString('base64')

/** The consent groups this loop is running under. */
const consentOf = (s: Settings | null) => s?.verbs ?? {}
 ...
 */
function start(s: Settings, logger?: (line: string) => void) {
  settings = s
 ...
 * @param {any[]} [override]
 */
async function publish(override?: any[]) {
  ...
  try {
    ...
    log(`[me-app] outbox: could not publish capabilities: ${e instanceof Error ? e.message : String(e)}`)
  }
}
...
async function loop() {
  ...
  while (running) {
    ...
    backoff = 1_000
    const command = await res.json().catch((): null => null)
    ...
    await fetch(`${settings.baseUrl}/api/v1/sensor/outbox/${command.id}/result`, {
      ...
      signal: AbortSignal.timeout(10_000),
    }).catch((e) => log(`[me-app] outbox: could not deliver result for ${command.kind}: ${e instanceof Error ? e.message : String(e)}`))
  }
}
...
export { start, stop, state, publish }
