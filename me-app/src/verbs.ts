...
import type { VerbConsent } from './types'
/**
...
/** One thing the assistant may ask this machine to do. */
interface Verb {
  name: string
  group: 'see' | 'act'
  /** Written for the LLM that must choose it. */
  description: string
  /** Name → what it means. */
...
const VERBS: Verb[] = [
  ...
  },
  {
    ...
    async run(args: any) {
      ...
      const matches = rank(text, tabs).slice(0, 8).map((t: any) => ({
  ...
  },
  {
    ...
    async run({ title, body }: { title: string; body: string }) {
  ...
  },
  {
    ...
    async run(args: any) {
      ...
      const found = recentFiles(within, (name: string) => !match || name.toLowerCase().includes(String(match).toLowerCase()))
      return {
 ...
 */
function catalog(consent: VerbConsent) {
 ...
 */
async function run(name: string, args: any, consent: VerbConsent) {
...
function parseWithin(text: string) {
 ...
 */
function recentFiles(withinMs: number, match: (name: string) => boolean) {
  const state = mounts.state()
  const results: Array<{ ref: string; name: string; folder: string; modified: number }> = []
  revealable.clear()
  ...
  function walk(dir: string, root: string, depth: number) {
    ...
    for (const entry of entries) {
      ...
      results.push({
        ref,
        name: entry.name,
        folder: path.dirname(full).replace(os.homedir(), '~'),
 ...
 */
function firstString(args: any, names: string[]) {
...
function describeArgs(args: any) {
...
function sanitize(text: string, max = 300) {
  return String(text ?? '')
 ...
 */
function rank(query: string, tabs: any[]) {
  ...
  const strong = scored.filter((x: any) => x.coverage >= 0.6)
  return (strong.length > 0 ? strong : scored).map((x: any) => x.tab)
}

/** Does any word in [haystack] start with [needle]? Cheap stemming, no dictionary. */
function wordPrefixMatch(haystack: string, needle: string) {
  let from = 0
  for (;;) {
