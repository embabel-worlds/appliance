...
const keyOf = (settings) => `${settings.baseUrl}|${settings.username}`
 ...
 */
async function pump(settings, key) {
...
const streaming = (settings) => Boolean(controller) && streamKey === keyOf(settings) && wanted
 ...
 */
async function send(settings, text) {
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      return { ok: false, message: e instanceof Error ? (e.cause?.message ?? e.message) : String(e) }
    }
 ...
 */
async function history(settings) {
  let res
  try {
    ...
    return { ok: false, message: e instanceof Error ? (e.cause?.message ?? e.message) : String(e), messages: [] }
  }
...
module.exports = { open, stop, send, history }
