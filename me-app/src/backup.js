 ...
 */
const docker = (args, cwd, timeout = 60_000) =>
  new Promise((resolve) => {
 ...
 * @returns {Promise<{ok: boolean, out: string}>}
 */
function streamToFile(args, file) {
 ...
 * @returns {Promise<{ok: boolean, out: string}>}
 */
function streamFromFile(args, file) {
  return new Promise((resolve) => {
...
async function copyOut(dir, dest) {
  const saved = []
