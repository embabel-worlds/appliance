 ...
 */
function resolveSource(uri: string | null | undefined) {
 ...
 * @returns {string | null}
 */
function toHostPath(containerPath: string) {
...
const tildeify = (p: string) => p.replace(os.homedir(), '~')
 ...
 */
async function watchProgress(settings: Settings, operationId: string, onStep: (step: any) => void) {
...
async function ask(settings: Settings, request: AskRequest, onStep: (step: any) => void) {
  ...
  try {
    ...
    const message = e instanceof Error ? ((e.cause as Error | undefined)?.message ?? e.message) : String(e)
  ...
  return {
    ...
    sources: ((payload.sources ?? []) as Array<Record<string, any>>).map((s) => ({ ...s, where: resolveSource(s.uri) })),
  }
}

export { ask, resolveSource }
