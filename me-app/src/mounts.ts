...
function write(dir: string, mounts: LocalMount[], env: Record<string, string> = {}) {
  const file = path.join(dir, OVERRIDE)
 ...
 */
function targetFor(host: string, taken: Set<string>) {
 ...
 */
function setIndex(host: string, index: boolean) {
  const current = state()
  if (!current.supported || !current.dir) return current
  const mounts = current.mounts.map((m: LocalMount) => (m.host === host ? { ...m, index } : m))
  write(current.dir, mounts, current.env)
  return { ...current, mounts }
}
