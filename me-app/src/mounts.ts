 ...
 *  the compiled dist/ directory finds it; a packaged app would need a setting. */
export function applianceDir(): string | null {
  let dir = __dirname
  for (let i = 0; i < 6; i++) {
...
function read(dir: string): LocalMount[] {
...
function write(dir: string, mounts: LocalMount[]): void {
  const file = path.join(dir, OVERRIDE)
  if (mounts.length === 0) {
...
export function state(): MountsState {
...
function targetFor(host: string, taken: Set<string>): string {
...
export function add(hosts: string[]): MountsState {
  const current = state()
...
export function remove(host: string): MountsState {
  const current = state()
  if (!current.supported || !current.dir) return current
...
const docker = (args: string[], cwd: string): Promise<{ ok: boolean; out: string }> =>
  new Promise((resolve) => {
 ...
 *  stays up throughout. */
export async function apply(): Promise<ConnectionResult> {
  const current = state()
