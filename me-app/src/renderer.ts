...
import type { Fact, GrantState, LocalMount, Settings, VerbConsent } from './types'
import { $ } from './dom'
...
$('grants').addEventListener('click', async () => {
  ...
  const granted = states.filter((g: GrantState) => g.state === 'granted').map((g: GrantState) => g.app)
  const denied = states.filter((g: GrantState) => g.state === 'denied').map((g: GrantState) => g.app)
  const parts = []
...
sendButton.addEventListener('click', async () => {
  const checkedIds = new Set(
    [...factList.querySelectorAll<HTMLElement>('input[type=checkbox]:checked')].map((c) => c.dataset['factId']),
  )
  const selected = facts.filter((f: Fact) => checkedIds.has(f.id))
...
function renderMounts(state: import('./types').MountsState) {
  mountList.innerHTML = ''
 ...
 */
function followIndexing(prefix: string) {
...
mountApply.addEventListener('click', async () => {
  ...
  if (result.ok && state.mounts.some((m: LocalMount) => m.index) && usernameInput.value.trim()) {
...
function setPill(ok: boolean, text: string) {
...
function paintChat(messages: ChatMessage[]) {
...
function cites(text: string) {
  return text.split(/(\[\d{1,3}\])/g).flatMap((part: string): Node[] => {
...
function renderAnswer(text: string) {
  paint(askAnswer, text, cites)
}

/** One source card: where it lives, what it said, and how to go look. */
function renderSource(source: DocSource) {
 ...
 */
function describeStep(step: AskStep) {
...
docDrop.addEventListener('drop', (e) => {
  ...
  void window.me.mountsState().then((state) => {
    ...
    const indexed = (state.mounts ?? []).find((m: LocalMount) => m.index) ?? (state.mounts ?? [])[0]
...
async function runIngest() {
  ...
  for (const [i, file] of files.entries()) {
    ...
    let result
    try {
      result = await window.me.uploadDocument(settings, file.name, await file.arrayBuffer(), tags.join(','))
    } catch (e) {
...
function fillModelOptions(select: Control, models: ModelInfo[]) {
  for (const group of ['LM Studio', 'Ollama']) {
    const inGroup = models.filter((m: ModelInfo) => m.provider === group)
  ...
  for (const model of models.filter((m: ModelInfo) => !LOCAL_PROVIDERS.has(m.provider))) {
    const option = document.createElement('option')
    option.value = model.name
...
async function loadModels() {
  ...
  const local = listed.models.filter((m: ModelInfo) => LOCAL_PROVIDERS.has(m.provider))
  const ready = local.filter((m: ModelInfo) => (loadedLocal as Record<string, unknown>)[m.name]).length
  setStatus(
    modelsStatus,
    true,
  ...
  const markChat = () => {
    const chosen = listed.models.find((m: ModelInfo) => m.name === chatSelect.value)
  ...
  for (const [roleId, description] of ROLES) {
    ...
    fillModelOptions(select as unknown as Control, listed.models)
    select.value = roles.roles?.[roleId]?.model ?? ''

    const badge = document.createElement('span')
    badge.className = 'local'
    const markLocal = () => {
      const chosen = listed.models.find((m: ModelInfo) => m.name === select.value)
 ...
 */
function paintIcon(tile: HTMLElement, settings: Settings, iconUrl: string | undefined) {
  if (!iconUrl) return
  ...
  if (text) {
    ...
    if (text !== firstLine || firstLine.length > 160) {
      ...
      toggle.addEventListener('click', () => {
        ...
        if (expanding && full.childNodes.length === 0) paint(full, text)
...
realmUpdateAll.addEventListener('click', async () => {
  ...
  if (!result.ok) {
    ...
    const updated = result.results.filter((r: RealmUpdateResult) => r.status === 'updated')
    const failed = result.results.filter((r: RealmUpdateResult) => r.status === 'error')
    const moved = updated.filter((r: RealmUpdateResult) => !/already up.to.date/i.test(r.summary ?? ''))
    const parts = [
      moved.length ? `${moved.map((r: RealmUpdateResult) => r.name).join(', ')} updated` : 'nothing new',
      failed.length ? `FAILED: ${failed.map((r: RealmUpdateResult) => `${r.name} (${r.message})`).join(', ')}` : '',
    ].filter(Boolean)
...
function privacyWords(inUse: ModelsInUse) {
  if (inUse.allLocal) {
    return {
  ...
  const cloud = inUse.models.filter((m: ModelInfo) => m.model && !m.local)
  const providers = inUse.cloudProviders.join(' and ')
  const surfaces = cloud.map((m: ModelInfo) => m.purpose.split(' —')[0]).join(' and ')
  return {
...
async function refreshPrivacy() {
  ...
  const label = (m: ModelInfo) => (m.model ? `${m.model}${m.local ? ' · on this Mac' : ` · ${m.provider}`}` : 'not set')
  const chat = inUse.models.find((m: ModelInfo) => m.purpose.startsWith('Chat'))
  const docs = inUse.models.find((m: ModelInfo) => m.purpose.startsWith('Documents'))
...
function renderPrivacyDetail(inUse: ModelsInUse) {
