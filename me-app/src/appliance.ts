 ...
 */
async function diagnose(baseUrl: string) {
 ...
 */
async function tryRun(cmd: string, args: string[], timeout: number, cwd?: string) {
  try {
...
async function imageCreated(id: string) {
  if (!id) return 0
...
export { diagnose, dockerInstalled, dockerRunning, update, DOCKER_URL }
