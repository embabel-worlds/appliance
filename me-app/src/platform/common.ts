...
/** A fact before numbering: everything but the `id`. */
 ...
 */
async function gather(collectors: Array<() => Promise<RawFact[]>>) {
...
async function jetbrainsRecents(roots: string[]) {
...
async function vscodeFolders(storageFiles: string[]) {
  const uris = new Set()
  ...
  const folders = [...uris]
    ...
    .map((u: string) => tildeify(decodeURIComponent(u.replace('file://', ''))))
 ...
 * @param {number} days
 */
const chromeCutoff = (days: number) =>
 ...
 */
async function chromiumHistoryFiles(userDataRoots: string[]) {
  const files = []
 ...
 */
async function querySnapshot(sqliteBin: string, historyFile: string, sql: string) {
 ...
 */
async function chromiumHistoryFacts(sqliteBin: string | null, userDataRoots: string[]) {
  if (!sqliteBin) return []
  const files = await chromiumHistoryFiles(userDataRoots)
  if (files.length === 0) return []

  /** @param {string} sql @returns {Promise<string[][]>} */
  const across = async (sql: string) => {
    const rows = []
