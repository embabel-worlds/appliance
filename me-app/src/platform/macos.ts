...
import type { RawFact } from './common'
...
/** @param {string} id */
const bundleName = (id: string) => BUNDLE_NAMES[id.toLowerCase()] ?? id
...
async function defaultHandlers() {
  ...
  const forScheme = (scheme: string) =>
    handlers.find((h: { LSHandlerURLScheme?: string; LSHandlerRoleAll?: string }) => h.LSHandlerURLScheme === scheme)?.LSHandlerRoleAll
  const facts = []
 ...
 */
async function isRunning(appName: string) {
  try {
 ...
 */
async function osascript(script: string, timeout = 5000) {
  try {
    const { stdout } = await run('osascript', ['-e', script], { timeout })
    const value = stdout.trim()
    return value ? { value } : {}
  } catch (e) {
    const message = (e as { stderr?: string; message?: string })?.stderr ?? (e as { message?: string })?.message ?? ''
 ...
 */
async function browserTab(front: string | undefined) {
...
function tabEnumScript(browser: { chromium?: boolean; app: string; proc: string }) {
 ...
 */
async function focusTab(ref: string) {
...
const macos = {
  ...
  async sampleFocus(tier1: boolean) {
  ...
  tier1Volatile(app: string | undefined) {
